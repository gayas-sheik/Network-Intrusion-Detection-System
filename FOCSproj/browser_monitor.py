"""
Real Browser Activity Monitor for NIDS
Actually monitors browser connections and detects tab opening/closing
"""
...
class BrowserMonitor:
    def __init__(self):
        ...
        self.thresholds = {
            'tab_spike': 5,  # 5 new tabs in 30 seconds
            'traffic_spike': 3.0,  # 3x normal traffic
            'persistent_connection': 300,  # 5 minutes
            'flood_detection': 20  # 20 connections in 10 seconds
        }
    ...
    def start_monitoring(self):
        ...
        if not self.running:
            ...
            print("🔍 Real browser monitoring started - detecting actual browser activity")
    ...
    def stop_monitoring(self):
        ...
        print("⏹️ Browser monitoring stopped")
            ...
            try:
                ...
                if new_connections:
                    for conn in new_connections:
                        ...
                        self._create_traffic_log(conn, 'new_connection')
        ...
        try:
            ...
            for conn in net_connections:
                if conn.status == 'ESTABLISHED' and conn.raddr:
                    ...
                    browser_ports = {80, 443, 8080, 3000, 5000, 8000, 8443, 9000, 3001, 4000, 6000, 7000}
                    
                    if (local_ip == self.my_ip and remote_port in browser_ports) or \
                       (remote_port in browser_ports and local_ip == self.my_ip):
    ...
    def _create_traffic_log(self, connection_key, action):
        ...
        try:
            ...
            # Prepare packet data
            packet_data = f"{traffic_data['source_ip']}:{traffic_data['source_port']} -> {traffic_data['dest_ip']}:{traffic_data['dest_port']} {traffic_data['protocol']} {traffic_data['size']} bytes [{action}]".encode('utf-8')
            ...
            log = TrafficLog(
                ...
                dest_ip=traffic_data['dest_ip'],
                source_port=traffic_data['source_port'],
                dest_port=traffic_data['dest_port'],
                protocol=traffic_data['protocol'],
                packet_size=traffic_data['size'],
    ...
    def _create_alert(self, alert_type, severity, source_ip, target_ip, description):
        ...
        try:
            ...
            alert_data = {
                ...
                'detection_method': 'browser_monitoring',
